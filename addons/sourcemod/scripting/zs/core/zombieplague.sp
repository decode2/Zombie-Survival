...
void GameEngineOnInit(/*void*/)
{
	...
	switch (GetEngineVersion(/*void*/))
	{
		case Engine_Unknown :    
		{
			LogError("Zombie Plague %s", ENGINE_UNKNOWN);
		}
		case Engine_Original :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_ORIGINAL);
		}
		case Engine_SourceSDK2006 :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_SOURCESDK2006);
		}
		case Engine_SourceSDK2007 :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_SOURCESDK2007);
		}
		case Engine_Left4Dead :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_LEFT4DEAD);
		}
		case Engine_DarkMessiah :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_DARKMESSIAH);
		}
		case Engine_Left4Dead2 :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_LEFT4DEAD2);
		}
		case Engine_AlienSwarm :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_ALIENSWARM);
		}
		case Engine_BloodyGoodTime :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_BLOODYGOODTIME);
		}
		case Engine_EYE :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_EYE);
		}
		case Engine_Portal2 :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_PORTAL2);
		}
		case Engine_CSGO :    
		{
		...
		case Engine_CSS :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_CSS);
		}
		case Engine_DOTA :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_DOTA);
		}
		case Engine_HL2DM :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_HL2DM);
		}
		case Engine_DODS :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_DODS);
		}
		case Engine_TF2 :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_TF2);
		}
		case Engine_NuclearDawn :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_NUCLEARDAWN);
		}
		case Engine_SDK2013 :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_SDK2013);
		}
		case Engine_Blade :    
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_BLADE);
		}
		case Engine_Insurgency :
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_INSURGENCY);
		}
		case Engine_Contagion :
		{
			LogError("Zombie Plague doesn't support: \"%s\"", ENGINE_CONTAGION);
		}
	}
...
stock void fnInitGameConfOffset(GameData gameConf, int &iOffset, char[] sKey)
{
	...
	{
		LogError("Failed to get offset: \"%s\"", sKey);
	}
}
...
stock void fnInitSendPropOffset(int &iOffset, char[] sClass, char[] sProp)
{
	...
	{
		LogError("Failed to find send prop: \"%s\"", sProp);
	}
}
...
stock Address fnCreateEngineInterface(GameData gameConf, char[] sKey, Address pAddress = Address_Null) 
{
	...
	if (hInterface == null) 
	{
		...
		{
			LogError( "Failed to load SDK call \"CreateInterface\". Update signature in \"%s\"", PLUGIN_CONFIG);
			return Address_Null;
		}
	}
	...
	{
		LogError("Failed to get pointer to interface %s(\"%s\")", sKey, sInterface);
		return Address_Null;
	}
