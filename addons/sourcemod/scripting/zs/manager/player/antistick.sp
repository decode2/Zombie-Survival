...
#if defined _antistick_included
	#endinput
#endif
#define _antistick_included
 ...
 **/
void AntiStick_OnMapStart(/*void*/){
 ...
 **/
void AntiStick_OnPluginStart(/*void*/){
	
	// Hook commands
	RegConsoleCmd("destrabar", AntiStickOnCommandCatched, "Destrabar cuando el personaje no pueda moverse");
	RegConsoleCmd("unstuck", AntiStickOnCommandCatched, "Unstucks player from the another prop.");
}
...
public void AntiStickOnStartTouch(int client, int entity)
{
	...
	if (/*StrContains(sEnt, "_projectile")  != -1 || StrContains(sEnt, "weapon")  != -1 || */StrContains(sEnt, AURA_CLASSNAME) != -1 || StrContains(sEnt, BLACKHOLE_EFFECT_NAME)){
		return;
	}
...
public Action AntiStickOnCommandCatched(int client, int iArguments)
{
	...
	{
        // Show block info
		TranslationPrintToChat(client, "Not stuck");
 ...
 **/
stock int AntiStickFindPlayerInSphere(int &it, float vPosition[3], float flRadius)
{
    // Initialize vector variables
 ...
 **/
stock bool AntiStickIsBoxIntersectingSphere(float flBoundaries[AntiStickBoxBound][3], float vPosition[3], float flRadius)
{
